$(document).ready(function(){
   ...
   /*
        모바일에서 1차 메뉴를 클릭하면 하위메뉴 메뉴가 열리는 기능
        1. 닫혀 있는 메뉴를 클릭하면 열림
            header .gnb .gnb_wrap ul.depth1 > li 에 open 클래스 추가
        2. 열려 있는 메뉴를 다시 클릭하면 닫힘
        3. 이미 하나ㅏ 열려있는 상태에서 다른 메뉴를 클릭하면
           이전에 열려있던 메뉴는 닫힘
        ----> 메뉴를 클릭했을때
              메뉴가 열려있는 상태라면 나 자신을 닫고 끝남
              메뉴가 닫혀있는 상태라면 다른 메뉴를 닫고 나만 열음
              :: 메뉴를 클릭했을때 현재 메뉴가 열려있는지 닫혀있는지 알아야함
              1차메뉴 li에 open 클래스가 있으면 열려있는것 ..
              hasClass

        4. 모바일에서 1차메뉴를 클릭하면 1차메뉴 a에 입력되어 있는 링크 주소로이동
           메뉴를 안열고 ...
           a태그에 있는 herf로 페이지 이동되는 것을 막아야함 ...
           a에 클릭했을때로 선택자 변경
